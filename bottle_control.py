...
def javascripts(filename):
    return static_file(filename, root = path + 'static/js')

@get('/<filename:re:.*\.css>')
def javascripts(filename):
    return static_file(filename, root = path + 'static/css')

@route('/')
def index():
    output = template(path + 'view.tpl')
    return output

@route('/timer', method='GET')
def ajax():
	return 'Since ' + now
